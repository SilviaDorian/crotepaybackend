...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: "Database error while fetching vouchers."
        });
...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        /**
         * IMPORTANT:
         * Only RAW token goes to frontend.
        ...
        res.status(500).json({
            error: "Server error during voucher creation."
        });
...
router.get('/public/:id', async (req, res) => {
    ...
    try {
        ...
        /**
         * Never expose token hashes publicly
         */
        ...
        const result = await query(
            `SELECT 
             ...
             LEFT JOIN public.users u
                ON v.creator_email = u.email
             WHERE v.id = $1
               AND v.recipient_access_token = $2`,
            [v_id, hashedToken]
        );
...
router.post('/release', async (req, res) => {
    ...
    try {
        ...
        if (v.status !== 'LOCKED') {
            throw new Error(`Funds are currently ${v.status}.`);
        }
        ...
        if (releaseKey) {
            ...
            if (v.release_key_hash !== hashedInput) {
        ...
        const diffInHours =
            (now - escrowStartTime) / (1000 * 60 * 60);
        ...
        await client.query(
            ...
            VALUES (
                ...
                $4,
                $5,
                'SUCCESSFUL'::text::voucher_status,
                $6
            )
            `,
            [
        ...
        res.json({
            success: true,
            message: `Funds moved to ${targetColumn}.`
        });
    ...
    try {
        ...
        await query(
            `UPDATE public.vouchers
             SET status = 'DISPUTED'::text::voucher_status,
                 description = $1,
                 updated_at = NOW()
             WHERE id = $2`,
