...
router.post('/flutterwave', async (req, res) => {
    ...
    try {
        ...
        if (payload.event === 'charge.completed') {
            ...
            const batchRef =
                payload.data.meta?.parent_batch_ref || txRef;

            console.log(
                `💳 WEBHOOK RECEIVED: ${txRef} | BATCH_REF: ${batchRef} | STATUS: ${paymentStatus}`
            );

            /**
             * =================================================
             * 🔒 BULK SAFETY GUARD (NEW)
            ...
            const alreadyProcessed =
                ...
                batchCheck.rows.every(v =>
                    ['LOCKED', 'RELEASED', 'AWAITING_SETTLEMENT']
                        .includes(v.status)
                );

            if (alreadyProcessed) {
                console.log(`⚠️ Batch already processed: ${batchRef}`);
                    ...
                    for (const v of batchResult.rows) {
                        ...
                        await client.query(
                            ...
                            [
                                ...
                                Number(v.amount),
                                v.currency,
                                `FLW-BATCH-${batchRef}-${v.id}`
                            ]
                        );

                        console.log(
                            `✅ Credited ${v.recipient_email} with ${v.amount} ${v.currency}`
                        );
                    }
            ...
            /**
             * =================================================
             * SINGLE VOUCHER PROCESSING (UNCHANGED)
             * =================================================
             */
            ...
            const amount =
                Number(payload.data.amount || voucher.amount);
